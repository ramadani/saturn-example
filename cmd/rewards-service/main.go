...
func main() {
	...
	consumerGroup, err := sarama.NewConsumerGroup(addresses, "rewards-service", saramaCog)
	if err != nil {
		log.Panic(err)
	}
	...
	log.Println("Rewards Service up and running!...")
...
type checkNewUserRewardsListener struct{}

func (l *checkNewUserRewardsListener) Handle(ctx context.Context, value []byte) (err error) {
	data := &userActivation{}
	_ = json.Unmarshal(value, data)

	log.Println("check new user rewards", data.Name)
	return
}
